...
mod tests {
    ...
    fn test_filter_butter_low_pass(#[case] filterImpl: FilterImpl) {
        ...
        let results_filename = format!(
            "{}_expected.txt",
            &fn_name[(fn_name.rfind(":").expect("Colon not found") + 1)..]
        );
        ...
        let (_, data) =
            load_test_file("bridge-strain-0-left-s-0-short.txt").expect("Could not load file");
    ...
    fn test_filter_butter_band_pass(#[case] filterImpl: FilterImpl) {
        ...
        let zpk =
            butter(order, FilterType::BandPass(cutoff_low, cutoff_hi), fs).expect("butter failed.");
        let sos = zpk2sos(&zpk, None).expect("zpk2sos failed");
    ...
    fn test_filter_butter_band_stop(#[case] filterImpl: FilterImpl) {
        ...
        let zpk =
            butter(order, FilterType::BandStop(cutoff_low, cutoff_hi), fs).expect("butter failed.");
        let sos = zpk2sos(&zpk, None).expect("zpk2sos failed");
