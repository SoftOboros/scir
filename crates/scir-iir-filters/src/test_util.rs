...
pub(crate) mod test {
    ...
    impl ZPKCoeffs {
        pub fn assert_approx_equal_to(&self, other: &ZPKCoeffs, eps: f64) {
            assert_eq!(
                self.z.len(),
                other.z.len(),
                "Number of zeroes are not equal."
            );
            assert_eq!(
                self.p.len(),
                other.p.len(),
                "Number of poles are not equal."
            );
        ...
        pub fn assert_approx_equal_to_with_sort(
            &self,
            other: &ZPKCoeffs,
            eps: f64,
            z_keys: &[Keys],
    ...
    pub fn load_test_file(file_name: &str) -> Result<(Vec<f64>, Vec<f64>), Error> {
        let pathname = format!(
            "{}/test_data/{}",
            env::var("CARGO_MANIFEST_DIR").expect("Env var CARGO_MANIFEST_DIR not found."),
            file_name
        );
