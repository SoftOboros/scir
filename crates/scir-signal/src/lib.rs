...
pub fn resample_poly(input: &Array1<f64>, up: usize, down: usize) -> Array1<f64> {
    assert!(up == 2 && down == 3);
    const H: [f64; 31] = [
        2.0745055596237081e-18, 3.5386000368802188e-03, 5.0683552343749614e-03,
        -4.3526285746599063e-18, -1.1612748844147112e-02, -1.7054042643607877e-02,
        1.0316832058578061e-17, 3.3827306077947913e-02, 4.6217484551914496e-02,
        -1.7688992996341970e-17, -8.4720082810181202e-02, -1.1660015080034160e-01,
        2.3653196480260126e-17, 2.6412611122005514e-01, 5.4460043997066177e-01,
        6.6521745601288684e-01, 5.4460043997066177e-01, 2.6412611122005514e-01,
        2.3653196480260126e-17, -1.1660015080034161e-01, -8.4720082810181220e-02,
        -1.7688992996341970e-17, 4.6217484551914517e-02, 3.3827306077947927e-02,
        1.0316832058578061e-17, -1.7054042643607898e-02, -1.1612748844147121e-02,
        -4.3526285746599063e-18, 5.0683552343749640e-03, 3.5386000368802188e-03,
...
mod tests {
    ...
    fn sosfilt_matches_fixture() {
        ...
        let sos_arr: Array2<f64> = ReadNpyExt::read_npy(File::open(base.join("butter_sos.npy")).unwrap()).unwrap();
        let sos = sos_from_array(&sos_arr);
        let input: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("sosfilt_input.npy")).unwrap()).unwrap();
        let expected: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("sosfilt_output.npy")).unwrap()).unwrap();
    ...
    fn filtfilt_matches_fixture() {
        ...
        let sos = sos_from_array(&sos_arr);
        let input: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("sosfilt_input.npy")).unwrap()).unwrap();
        let expected: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("filtfilt_output.npy")).unwrap()).unwrap();
    ...
    fn resample_poly_matches_fixture() {
        let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fixtures");
        let input: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("sosfilt_input.npy")).unwrap()).unwrap();
        let expected: Array1<f64> = ReadNpyExt::read_npy(File::open(base.join("resample_poly_output.npy")).unwrap()).unwrap();
